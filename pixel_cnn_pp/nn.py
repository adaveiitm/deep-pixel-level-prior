...
def discretized_mix_logistic_loss(x, l, sum_all=True, return_prob=True):
    ...
    # The below line is for testing with the maximum of logistic
    # Comment this for traditional Pixel-CNN PP
    #temp = 1e-5
    #sel = tf.nn.softmax((logit_probs - tf.log(-tf.log(tf.random_uniform(logit_probs.get_shape(),
                                                    # minval=1e-5, maxval=1. - 1e-5))))/temp,dim=3)
    #sel_logit = logit_probs * sel
    ...
    #log_probs = tf.reduce_sum(log_probs, 3) + log_prob_from_logits(sel_logit)
    ...
    if sum_all:
        if return_prob:
            ...
            # return -tf.reduce_sum(log_sum_exp(log_probs)), log_prob_from_logits(logit_probs)
...
def sample_from_discretized_mix_logistic(l, nr_mix, x=None):
    ls = int_shape(l)
    xs = ls[:-1] + [3]
    # unpack parameters
    logit_probs = l[:, :, :, :nr_mix]
    ...
    np.random.seed(123)
    sel_argmax = tf.argmax(logit_probs - tf.log(-tf.log(tf.random_uniform(
        logit_probs.get_shape(), minval=1e-5, maxval=1. - 1e-5))), 3)
    sel = tf.one_hot(sel_argmax, depth=nr_mix, dtype=tf.float32)
    sel_gumbel = tf.contrib.distributions.RelaxedOneHotCategorical(temperature=0.00001,logits=logit_probs).sample(seed=123)
    temp = 0.0001
    sel = tf.nn.softmax(logit_probs - tf.log(-tf.log(tf.random_uniform(
        logit_probs.get_shape(), minval=1e-5, maxval=1. - 1e-5)))/temp,dim=3)
    ...
    sampled_x = tf.concat([tf.reshape(x0, xs[:-1] + [1]), tf.reshape(x1, xs[:-1] + [1]), tf.reshape(x2, xs[:-1] + [1])], 3)
    return tf.gradients(sampled_x, [x])
